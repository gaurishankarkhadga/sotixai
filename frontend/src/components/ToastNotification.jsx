...
function ToastItem({ toast, onRemove }) {
    ...
    // Simplify JSON or long messages to make it shorter and cleaner
    let displayMessage = toast.message || toast.title || '';
    if (typeof displayMessage === 'string' && (displayMessage.startsWith('{') || displayMessage.startsWith('['))) {
        try {
            const parsed = JSON.parse(displayMessage);
            displayMessage = parsed.message || parsed.error || 'System updated';
        } catch (e) {
    ...
    if (typeof displayMessage === 'string') {
        displayMessage = cleanText(displayMessage);
    ...
    return (
        <div className={`toast-item toast-${type} ${isExiting ? 'toast-exit' : 'toast-enter'}`}>
            <div className="toast-content">
                <span className="toast-title" style={{ fontSize: '13px', fontWeight: 500 }}>
                    {displayMessage}
                </span>
            </div>
