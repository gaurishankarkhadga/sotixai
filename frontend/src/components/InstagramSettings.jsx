...
function InstagramSettings() {
    ...
    const [storyMentionMessage, setStoryMentionMessage] = useState('Thank you so much for the mention! ❤');
    ...
    const fetchDmAutoReplySettings = async () => {
        try {
            ...
            if (data.success) {
                ...
                setStoryMentionMessage(data.data.storyMentionMessage || 'Thank you so much for the mention! ❤');
    ...
    const getActionBadge = (action) => {
        if (action === 'hidden') return { icon: '', text: 'Hidden', className: 'badge-hidden' };
        if (action === 'skipped') return { icon: '⏭', text: 'Skipped', className: 'badge-skipped' };
        return { icon: '', text: 'Replied', className: 'badge-replied' };
    };

    const getAssetTypeIcon = (type) => {
        const icons = { product: '', link: '', course: '📚', image: '', service: '⚙', ebook: '📖', merch: '👕' };
        return icons[type] || '📎';
    };

    const getReplyTypeLabel = (type) => {
        const labels = { text: ' Text', text_with_link: ' Text + Link', image: ' Image', product_recommendation: ' Product' };
                ...
                {!token ? (
                    ...
                    <div className="data-section">
                                        ...
                                        {autoReplyLog.map((entry, i) => {
                                            ...
                                            return (
                                                <div key={entry.commentId + '-' + i} className="log-entry">
                                                    ...
                                                    <p className="log-reply">↩ "{entry.replyText}"</p>
                        ...
                        <div className="auto-reply-section dm-section">
                            <div className="auto-reply-header">
                                <h2>✉ AI-Powered DM Auto-Reply</h2>
                                ...
                                {storyMentionEnabled && (
                                    <div className="setting-row">
                                        <label>
                                            ...
                                            <input type="text" value={storyMentionMessage} onChange={(e) => setStoryMentionMessage(e.target.value)} placeholder="Thank you so much for the mention! ❤" maxLength={200} className="reply-input" />
                                        </label>
                                    </div>
                                )}
                                    ...
                                    {showAssetForm && (
                                        <div className="asset-form">
                                            ...
                                            <button onClick={saveAsset} disabled={assetSaving || !assetForm.title.trim()} className="btn-save">
                                                {assetSaving ? 'Saving...' : editingAssetId ? '✏ Update Asset' : '➕ Add Asset'}
                                            </button>
                                        </div>
                                    )}
                                            ...
                                            {creatorAssets.map(asset => (
                                                <div key={asset._id} className={`asset-card ${!asset.isActive ? 'asset-inactive' : ''}`}>
                                                    ...
                                                    <div className="asset-actions">
                                                        <button onClick={() => toggleAssetActive(asset)} className="btn-tiny">{asset.isActive ? '🟢 Active' : '⏸ Paused'}</button>
                                                        <button onClick={() => editAsset(asset)} className="btn-tiny">✏ Edit</button>
                                                        <button onClick={() => deleteAsset(asset._id)} className="btn-tiny btn-danger">🗑</button>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                ...
                                <div className="log-header">
                                    ...
                                    <div className="log-actions">
                                        ...
                                        {dmAutoReplyLog.length > 0 && (
                                            <button onClick={clearDmAutoReplyLog} className="btn-small btn-danger">🗑 Clear</button>
                                        )}
                                    </div>
                                </div>
                                {dmAutoReplyLog.length === 0 ? (
