...
// (Legacy scheduleDMAutoReply removed completely — handled by instaautomation_dm_automate)
            ...
            for (const entry of body.entry) {
                ...
                for (const change of changes) {
                    if (change.field === 'comments') {
                        ...
                        commentToDmHandled = await processCommentToDm(igUserId, commentData, helpers);
...
router.post('/debug/test-dm-webhook', async (req, res) => {
    try {
        ...
        // Run the DM auto-reply flow using the new modularized logic
        const mockEvent = {
            sender: { id: testMessageData.senderId },
            recipient: { id: testMessageData.recipientId },
            message: {
                mid: testMessageData.id,
                text: testMessageData.text,
                attachments: []
            },
            timestamp: testMessageData.timestamp
        };
        
        await processDirectMessage(mockEvent, userId, {
            resolveUserIdMapping,
            acquireConversationLock,
            sendDirectMessage,
            sendGenericTemplate
