    ...
    async execute(intent, params, context) {
        ...
        try {
            ...
            if (intent === 'enable_comment_to_dm') {
                ...
                if (useAssets) {
                    if (allAssets.length > 0) {
                        ...
                        if (params.forceAssetId) {
                            const matched = allAssets.find(a => a._id.toString() === params.forceAssetId);
                            if (matched) {
                                matchedInfo = {
                            ...
                            try {
                                ...
                                // Check if they requested a specific product that does not exist at all
                                if (matchResult.isUnavailableRequest && !matchResult.isGenericMessage) {
                                    return {
                                        success: true, // We successfully detected what they want
                                        intent: 'request_missing_asset',
                                        message: "I see you want to promote a product or link, but it's not in your Assets tab yet. Please provide the details below so I can set up the automation!",
                                        data: {
                                            originalParams: params,
                                            suggestedTitle: matchResult.matchReason || creatorPrompt.substring(0, 30)
                                        }
                                    };
                                }
                        ...
                        if (!matchedInfo && !params.forceAssetId) {
                            return {
                                success: true,
                                intent: 'request_missing_asset',
                                message: "I see you want to promote a product or link for this automation, but I couldn't identify a specific one in your library. Please provide the details below so I can set it up!",
                                data: {
                                    originalParams: params,
                                    suggestedTitle: creatorPrompt ? creatorPrompt.substring(0, 30) : ''
                                }
                            };
                        }
                    } else {
                        // If they want to use assets but the library is completely empty
                        const creatorPrompt = context.originalMessage || '';
                        
                        if (params.forceAssetId) {
                            // Should not happen since we just added an asset if forceAssetId is passed
                            matchedInfo = null; 
                        } else {
                            return {
                                success: true,
                                intent: 'request_missing_asset',
                                message: "I see you want to promote a product or link for this automation, but your Assets library is currently empty. Please provide the details below so I can set it up!",
                                data: {
                                    originalParams: params,
                                    suggestedTitle: creatorPrompt ? creatorPrompt.substring(0, 30) : ''
                                }
            ...
            if (intent === 'configure_comment_to_dm') {
                ...
                const existingSetting = await CommentToDmSetting.findOne({ userId });
                const isUsingAssets = update.useAssets !== undefined ? update.useAssets : (existingSetting ? existingSetting.useAssets : false);
                ...
                if (isUsingAssets) {
                    ...
                    if (!matchedInfo && !params.forceAssetId) {
                        return {
                            success: true,
                            intent: 'request_missing_asset',
                            message: "You are trying to update an automation to share a link, but no product is linked. Please provide the link details below!",
                            data: {
                                originalParams: params,
                                suggestedTitle: ''
                            }
                ...
                return {
                    ...
                    data: { ...setting.toObject(), automationType: 'comment_to_dm', matchedAsset: matchedInfo }
                };
            }
